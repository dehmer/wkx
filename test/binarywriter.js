...
describe('wkx', function () {
    describe('BinaryWriter', function () {
        it('writeVarInt - 1', function () {
            ...
            assert.strictEqual(binaryWriter.buffer.toString('hex'), '01');
        ...
        it('writeVarInt - 300', function () {
            ...
            assert.strictEqual(binaryWriter.buffer.toString('hex'), 'ac02');
            assert.strictEqual(length, 2);
        });
        it('writeUInt8 - enough space', function () {
            var binaryWriter = new BinaryWriter(1);
            binaryWriter.writeUInt8(1);
            assert.strictEqual(binaryWriter.buffer.length, 1);
            assert.strictEqual(binaryWriter.position, 1);
        });
        ...
        it('writeUInt16LE - not enough space  / allow resize', function () {
            ...
            assert.strictEqual(binaryWriter.buffer.length, 2);
            assert.strictEqual(binaryWriter.position, 2);
        });
    });
});
